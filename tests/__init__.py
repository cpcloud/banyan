...
from banyan import *
from .testbase import UnittestBase
import leak_detector
import banyan._trace
import min_max_updator
...
_updators = [None, RankUpdator, MinGapUpdator, min_max_updator.MinMaxUpdator]
...
class TestBasics(unittest.TestCase, UnittestBase):
    def test_00(self):
...
class Test01BasicSet(unittest.TestCase, UnittestBase):
    def test_00(self):
    ...
    def _check_tree_ok(self, r_, tree, builtin_):
        ...
        if len(builtin_) > 0:
            ...
            if hasattr(tree, 'min_gap'):
                ...
                if len(keys) > 1:
                    builtin_min_gap = \
                        min([abs(x - y) for (x, y) in zip(keys[1: ], keys[: -1])])
                    self.assertEqual(
                        tree.min_gap(),
                        builtin_min_gap,
